...
//uIP TCP Server测试 代码	
extern uint8_t slave_id_cnt;
extern uint8_t slave_id;
extern uint8_t receive_slave_data[21][26];
extern uint8_t USART_RX_BUF[25];
extern uint8_t signal_intensity_flag;
extern uint8_t signal_data[21][6];			    
...
{
	struct tcp_demo_appstate *s = (struct tcp_demo_appstate *)&uip_conn->appstate;
//	if(signal_intensity_flag == 0)
//	{
		s->textptr = receive_slave_data[slave_id];
	  s->textlen = 26;
//	}
//	if(signal_intensity_flag == 1)
//	{
//		s->textptr = signal_data[slave_id_cnt];
//	  s->textlen = 6;
//	}
