...
static void ENC28J60_SPI2_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOA,ENABLE );//PORTB,D,G时钟使能 
 
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;				 // 端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
 	GPIO_Init(GPIOA, &GPIO_InitStructure);					 //根据设定参数初始化GPIOD.2
 	GPIO_SetBits(GPIOA,GPIO_Pin_4);						 //PD.2上拉  ETH_INT
...
// 	GPIO_SetBits(GPIOB,GPIO_Pin_12);						 //PB.12上拉
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11|GPIO_Pin_12;//PG6/7/8 推挽  上拉			 
 	GPIO_Init(GPIOA, &GPIO_InitStructure);					 //根据设定参数初始化//PG6/7/8
 	GPIO_SetBits(GPIOA,GPIO_Pin_11|GPIO_Pin_12);//PG6/7/8上拉	
